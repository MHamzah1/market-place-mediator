      ...
      <div className="max-w-7xl mx-auto px-3 sm:px-4 md:px-6 lg:px-8">
        ...
        <div className="flex flex-wrap justify-center gap-2 sm:gap-3 md:gap-4 mb-8 sm:mb-10 md:mb-12 px-2">
          {categories.map((category) => (
            <button
              ...
              className={`px-4 sm:px-6 md:px-8 py-2 sm:py-2.5 md:py-3 rounded-xl sm:rounded-2xl font-bold text-xs sm:text-sm md:text-base transition-all duration-200 ${
                category === selectedCategory
                  ? isDarkMode
                    ? "bg-gradient-to-r from-yellow-500 to-orange-600 text-white shadow-lg sm:shadow-xl shadow-yellow-500/30"
                    : "bg-gradient-to-r from-yellow-500 to-orange-600 text-white shadow-lg sm:shadow-xl shadow-yellow-500/30"
                  : isDarkMode
        ...
        {featuredLoading && (
          <div className="flex justify-center items-center py-12 sm:py-16 md:py-20">
            <div className="animate-spin rounded-full h-12 w-12 sm:h-14 sm:w-14 md:h-16 md:w-16 border-4 border-yellow-500 border-t-transparent"></div>
          </div>
        )}
        ...
        {!featuredLoading && filteredListings.length === 0 && (
          <div
            className={`text-center py-12 sm:py-16 md:py-20 rounded-xl sm:rounded-2xl md:rounded-3xl ${
              isDarkMode ? "bg-slate-900" : "bg-white"
            } shadow-lg sm:shadow-xl`}
          >
            <AiOutlineCar
              className={`text-5xl sm:text-6xl md:text-7xl lg:text-8xl mx-auto mb-4 sm:mb-6 ${
            ...
            {filteredListings.map((car) => (
              <Link
                ...
                className={`rounded-xl sm:rounded-2xl md:rounded-3xl shadow-lg sm:shadow-xl md:shadow-2xl transition-all duration-300 overflow-hidden group cursor-pointer border-2 ${
                  ...
                  hoveredCard === car.id
                    ? "transform -translate-y-2 sm:-translate-y-3 md:-translate-y-4 scale-105"
                    : ""
                }`}
              ...
              >
                ...
                >
                  {car.images && car.images[0] ? (
                    ...
                    <div className="flex items-center justify-center h-full">
                      <span className="text-6xl sm:text-7xl md:text-8xl lg:text-9xl group-hover:scale-125 transition-transform duration-500">
                        🚗
                      </span>
                    </div>
                  )}
                  ...
                  <div className="absolute top-2 sm:top-3 md:top-4 left-2 sm:left-3 md:left-4">
                    <FeaturedBadge
                      badge={car.featuredBadge || "Unggulan"}
                      size="md"
                    />
                  </div>
                  ...
                  <button
                    ...
                    className={`absolute top-2 sm:top-3 md:top-4 right-2 sm:right-3 md:right-4 w-8 h-8 sm:w-9 sm:h-9 md:w-10 md:h-10 rounded-full flex items-center justify-center shadow-lg sm:shadow-xl backdrop-blur-sm transition-all duration-200 group/fav ${
                      isDarkMode
                  ...
                  {/* Views Badge */}
                  <div className="absolute bottom-2 sm:bottom-3 md:bottom-4 right-2 sm:right-3 md:right-4 flex items-center gap-1 px-2 sm:px-2.5 md:px-3 py-1 bg-black/50 backdrop-blur-sm rounded-full text-white text-xs sm:text-sm">
                ...
                <div className="p-4 sm:p-5 md:p-6">
                  <h3
                    className={`font-black text-base sm:text-lg md:text-xl mb-2 sm:mb-3 transition-colors ${
                      isDarkMode
                  ...
                  <div
                    className={`flex items-center text-xs sm:text-sm mb-4 sm:mb-5 md:mb-6 gap-3 sm:gap-4 md:gap-6 ${
                  ...
                  >
                    ...
                    <span className="flex items-center gap-1 sm:gap-2">
                      ...
                      <span className="sm:hidden">
                        {car.locationCity.substring(0, 10)}...
                      </span>
                    </span>
                  </div>
                  ...
                  <div className="grid grid-cols-3 gap-2 sm:gap-2.5 md:gap-3 mb-4 sm:mb-5 md:mb-6">
                    <div
                      className={`text-center p-2 sm:p-2.5 md:p-3 rounded-lg sm:rounded-xl ${
                        isDarkMode ? "bg-slate-800" : "bg-gray-50"
                      }`}
                    >
                      <BsSpeedometer2
                        className={`text-sm sm:text-base md:text-lg mx-auto mb-1 ${
                  ...
                  {/* Price & Action */}
                  <div
                    className={`flex items-center justify-between pt-4 sm:pt-5 md:pt-6 border-t-2 ${
                  ...
                  >
                    <div>
                      ...
                      <div
                        className={`text-sm sm:text-base md:text-lg lg:text-xl xl:text-2xl font-black ${
                    ...
                    <button
                      onClick={(e) => e.preventDefault()}
                      className="flex items-center gap-1 sm:gap-2 px-2 sm:px-3 md:px-4 py-1.5 sm:py-2 bg-green-500 text-white font-bold text-xs sm:text-sm rounded-lg sm:rounded-xl hover:bg-green-600 transition-all"
                    >
                      <AiOutlineWhatsApp className="text-sm sm:text-base md:text-lg" />
        ...
        {showViewAll && filteredListings.length > 0 && (
          <div className="text-center mt-8 sm:mt-12 md:mt-16">
            <Link
              href="/marketplace"
              className={`px-6 sm:px-8 md:px-10 py-3 sm:py-3.5 md:py-4 rounded-xl sm:rounded-2xl font-bold text-sm sm:text-base transition-all duration-200 inline-flex items-center space-x-2 sm:space-x-3 border-2 ${
                isDarkMode
                  ? "border-yellow-500 text-yellow-400 hover:bg-yellow-500 hover:text-white"
                  : "border-yellow-500 text-yellow-600 hover:bg-yellow-500 hover:text-white"
              } hover:scale-105 shadow-lg sm:shadow-xl`}
            >
              <AiOutlineCar className="text-lg sm:text-xl md:text-2xl" />
              <span className="hidden sm:inline">Lihat Semua Mobil</span>
