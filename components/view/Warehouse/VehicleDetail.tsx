...
const VehicleDetail = ({ id }: { id: string }) => {
  ...
  return (
    <div className={`space-y-6 max-w-7xl mx-auto px-2 sm:px-4 pb-12`}>
      ...
      <div className={`p-4 rounded-2xl border shadow-sm ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-white border-slate-200"}`}>
        <div className="flex flex-col md:flex-row items-start md:items-center justify-between gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2 mb-1">
                <span className={`px-2.5 py-0.5 rounded-md text-xs font-bold border ${sc.color}`}>
                  {sc.label}
                </span>
              ...
              <h1 className={`text-xl sm:text-2xl font-bold ${isDark ? "text-white" : "text-slate-900"}`}>
                {vehicle.brandName} {vehicle.modelName} <span className="text-emerald-500">{vehicle.year}</span>
              </h1>
            </div>
          </div>
          ...
          <div className="flex flex-wrap gap-2 w-full md:w-auto">
            {vehicle.status === "inspecting" && (
              <Link
                href={generateUrlWithEncryptedParams("/warehouse/inspections/create", { vehicleId: vehicle.id })}
            ...
            {vehicle.status === "registered" && (
              <button
                ...
                className="flex flex-1 md:flex-none justify-center items-center gap-2 px-5 py-2.5 rounded-xl bg-blue-500/20 text-blue-600 dark:text-blue-400 font-semibold text-sm hover:bg-blue-500/30 transition-colors border border-blue-500/30 disabled:opacity-50"
              >
                <FiDollarSign /> Bayar Tagihan Admin
              </button>
            )}
            {(vehicle.status === "in_warehouse" || vehicle.status === "in_repair") && (
              <>
                <Link
                  href={generateUrlWithEncryptedParams("/warehouse/repairs/create", { vehicleId: vehicle.id })}
                ...
                <button
                  onClick={() => dispatch(updateVehicleStatus({ id: vehicle.id, status: "ready" }))}
                  disabled={actionLoading}
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        <div className="lg:col-span-2 space-y-6">
          ...
          {/* Main Image Gallery */}
          <div className={`p-4 sm:p-5 rounded-2xl border shadow-sm ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-white border-slate-200"}`}>
            <div
          ...
          <div className={`grid grid-cols-2 sm:grid-cols-4 gap-3 p-4 rounded-2xl border ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-white border-slate-200"} shadow-sm`}>
            {[
              { label: "Kilometer", val: `${vehicle.mileage.toLocaleString("id-ID")} km`, icon: BsSpeedometer2, color: "text-blue-500" },
              { label: "Transmisi", val: vehicle.transmission || "N/A", icon: TbManualGearbox, color: "text-purple-500" },
              { label: "Bahan Bakar", val: vehicle.fuelType, icon: BsFuelPump, color: "text-orange-500" },
              { label: "Tahun", val: vehicle.year.toString(), icon: FiCalendar, color: "text-emerald-500" },
            ].map((stat, i) => (
              <div key={i} className={`flex flex-col items-center justify-center p-3 rounded-xl ${isDark ? "bg-slate-700/30" : "bg-slate-50"}`}>
                <stat.icon className={`text-2xl mb-2 ${stat.color}`} />
                <span className={`text-[10px] sm:text-xs font-medium uppercase tracking-wider mb-1 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
                  {stat.label}
                </span>
                <span className={`text-sm sm:text-base font-bold capitalize ${isDark ? "text-white" : "text-slate-900"}`}>
                  {stat.val}
                </span>
              </div>
            ))}
          ...
          <div className={`p-5 sm:p-6 rounded-2xl border shadow-sm ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-white border-slate-200"}`}>
            ...
            <div className={`prose prose-sm max-w-none ${isDark ? "prose-invert text-slate-300" : "text-slate-700"} whitespace-pre-wrap leading-relaxed`}>
              {vehicle.description || <span className="italic opacity-50">Tidak ada deskripsi yang ditambahkan.</span>}
            </div>
            
            {vehicle.notes && (
              <div className={`mt-6 p-4 rounded-xl border ${isDark ? "bg-yellow-500/10 border-yellow-500/20 text-yellow-200" : "bg-yellow-50 border-yellow-200 text-yellow-800"}`}>
        ...
        <div className="space-y-6">
          {/* Price Card */}
          <div className={`p-5 sm:p-6 rounded-2xl border shadow-sm ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-white border-slate-200"}`}>
            <h3 className={`text-sm font-semibold uppercase tracking-wider mb-2 ${isDark ? "text-slate-400" : "text-slate-500"}`}>
            ...
            {(vehicle.locationCity || vehicle.locationProvince) && (
              <div className={`flex items-center gap-2 text-sm font-medium ${isDark ? "text-slate-300" : "text-slate-700"}`}>
                ...
                {vehicle.locationCity && <span className="capitalize">{vehicle.locationCity}</span>}
                {vehicle.locationCity && vehicle.locationProvince && <span>, </span>}
                {vehicle.locationProvince && <span className="capitalize">{vehicle.locationProvince}</span>}
              </div>
            )}
          </div>
          ...
          <div className={`p-5 sm:p-6 rounded-2xl border shadow-sm ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-white border-slate-200"}`}>
            ...
            <div className="space-y-4">
              <div className="flex justify-between items-center text-sm py-2 border-b border-slate-100 dark:border-slate-700">
                <span className={isDark ? "text-slate-400" : "text-slate-500"}>Nama</span>
                <span className={`font-semibold ${isDark ? "text-white" : "text-slate-900"}`}>{vehicle.sellerName}</span>
              </div>
              <div className="flex justify-between items-center text-sm py-2 border-b border-slate-100 dark:border-slate-700">
                <span className={isDark ? "text-slate-400" : "text-slate-500"}>Phone / WA</span>
                <span className={`font-semibold ${isDark ? "text-white" : "text-slate-900"}`}>
                  {vehicle.sellerPhone} {vehicle.sellerWhatsapp ? ` / ${vehicle.sellerWhatsapp}` : ""}
                </span>
              </div>
              {vehicle.sellerKtp && (
                 <div className="flex justify-between items-center text-sm py-2 border-b border-slate-100 dark:border-slate-700">
                  <span className={isDark ? "text-slate-400" : "text-slate-500"}>No KTP</span>
                  <span className={`font-semibold font-mono ${isDark ? "text-slate-300" : "text-slate-700"}`}>{vehicle.sellerKtp}</span>
                </div>
              )}
            </div>
          </div>
          ...
          <div className={`p-5 sm:p-6 rounded-2xl border shadow-sm ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-white border-slate-200"}`}>
            <h2 className={`text-lg font-bold mb-4 flex items-center gap-2 ${isDark ? "text-white" : "text-slate-900"}`}>
              <AiOutlineSafety className="text-rose-500 text-xl" /> Legalitas & Status
            </h2>
            <div className="space-y-3">
               <InfoRow isDark={isDark} label="Plat Nomor" value={vehicle.licensePlate} />
               <InfoRow isDark={isDark} label="Pajak" value={vehicle.taxStatus || "N/A"} />
               <InfoRow isDark={isDark} label="Kepemilikan" value={vehicle.ownershipStatus || "N/A"} />
               <InfoRow isDark={isDark} label="Nomor Rangka" value={vehicle.chassisNumber} mono />
               <InfoRow isDark={isDark} label="Nomor Mesin" value={vehicle.engineNumber} mono />
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mt-8">
        ...
        <div className={`p-5 sm:p-6 rounded-2xl border shadow-sm ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-white border-slate-200"}`}>
          ...
          {inspections.length === 0 ? (
            <div className={`py-8 text-center rounded-xl border border-dashed ${isDark ? "border-slate-700 text-slate-500" : "border-slate-300 text-slate-400"}`}>
              <p>Belum ada riwayat inspeksi.</p>
            </div>
          ) : (
            <div className="space-y-4">
              {inspections.map((insp) => (
                <div key={insp.id} className={`p-4 rounded-xl border ${isDark ? "bg-slate-700/30 border-slate-600/50" : "bg-slate-50 border-slate-100"}`}>
                  <div className="flex justify-between items-start mb-3">
                    <div>
                      <span className={`text-sm font-bold capitalize ${isDark ? "text-white" : "text-slate-900"}`}>
                        {insp.inspectionType.replace("_", " ")}
                      </span>
                      <span className={`ml-2 px-2.5 py-0.5 rounded-md text-[10px] font-bold uppercase tracking-wider ${
                        insp.overallResult === "accepted_ready" ? "bg-green-500/20 text-green-500 dark:text-green-400"
                        : insp.overallResult === "accepted_repair" ? "bg-orange-500/20 text-orange-500 dark:text-orange-400"
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                     {[
                      ...
                      { label: "Chs", score: insp.chassisScore },
                        ...
                        <div key={idx} className={`px-2 py-1 rounded-md text-xs font-medium ${isDark ? "bg-slate-800 text-slate-300" : "bg-white border text-slate-700"}`}>
                           {item.label}: <strong className={isDark ? "text-white" : "text-slate-900"}>{item.score ?? "-"}</strong>
                        </div>
                     ))}
                  </div>
                  ...
                  <div className="flex flex-wrap gap-1.5 text-[10px] font-bold uppercase">
                     {insp.hasBpkb && <span className="px-2 py-1 rounded bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20">BPKB ✓</span>}
                     {insp.hasStnk && <span className="px-2 py-1 rounded bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20">STNK ✓</span>}
                     {insp.hasFaktur && <span className="px-2 py-1 rounded bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20">Faktur ✓</span>}
                     {insp.hasKtp && <span className="px-2 py-1 rounded bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20">KTP ✓</span>}
                     {insp.hasSpareKey && <span className="px-2 py-1 rounded bg-green-500/10 text-green-600 dark:text-green-400 border border-green-500/20">Kunci Cadangan ✓</span>}
                  </div>

                  {insp.repairNotes && <p className="text-xs text-orange-600 dark:text-orange-400 mt-3 font-medium">Perbaikan Diperlukan: {insp.repairNotes}</p>}
                  {insp.rejectionReason && <p className="text-xs text-red-600 dark:text-red-400 mt-3 font-medium">Alasan Tolak: {insp.rejectionReason}</p>}
                </div>
              ))}
            </div>
          )}
        ...
        <div className={`p-5 sm:p-6 rounded-2xl border shadow-sm ${isDark ? "bg-slate-800/50 border-slate-700/50" : "bg-white border-slate-200"}`}>
          ...
          {repairs.length === 0 ? (
           <div className={`py-8 text-center rounded-xl border border-dashed ${isDark ? "border-slate-700 text-slate-500" : "border-slate-300 text-slate-400"}`}>
            ...
            <div className="space-y-4">
              {repairs.map((r) => (
                <div key={r.id} className={`p-4 rounded-xl border flex flex-col gap-2 ${isDark ? "bg-slate-700/30 border-slate-600/50" : "bg-slate-50 border-slate-100"}`}>
                  <div className="flex justify-between items-start">
                    <div className="flex items-center gap-2">
                      <span className={`px-2.5 py-0.5 rounded-md text-[10px] uppercase font-bold tracking-wider ${
                        r.repairType === "light" ? "bg-blue-500/20 text-blue-600 dark:text-blue-400" : "bg-red-500/20 text-red-600 dark:text-red-400"
                      }`}>
                        {r.repairType === "light" ? "Ringan" : "Berat"}
                      </span>
                      <span className={`px-2.5 py-0.5 rounded-md text-[10px] uppercase font-bold tracking-wider ${
                         r.status === "completed" ? "bg-green-500/20 text-green-600 dark:text-green-400"
                         : r.status === "in_progress" ? "bg-yellow-500/20 text-yellow-600 dark:text-yellow-400"
                         : r.status === "cancelled" ? "bg-red-500/20 text-red-600 dark:text-red-400"
                  ...
                  <div className="flex gap-4 mt-2">
                    {r.estimatedCost && (
                      <div className="text-xs">
                        <span className={isDark ? "text-slate-500" : "text-slate-400"}>Estimasi: </span>
                        <span className={`font-semibold ${isDark ? "text-slate-300" : "text-slate-700"}`}>{formatPrice(r.estimatedCost)}</span>
                      </div>
                    )}
                    {r.actualCost && (
                      <div className="text-xs">
                         <span className={isDark ? "text-slate-500" : "text-slate-400"}>Aktual: </span>
                         <span className="font-semibold text-emerald-500">{formatPrice(r.actualCost)}</span>
                      </div>
                    )}
                  </div>
                </div>
              ...
              {images.map((_, idx) => (
                <button
                  ...
                  className={`w-2.5 h-2.5 rounded-full transition-all flex-shrink-0 ${
                    currentImageIndex === idx ? "bg-emerald-500 scale-125" : "bg-white/40 hover:bg-white/60"
                  }`}
                />
              ))}
            </div>
...
function InfoRow({ label, value, isDark, mono }: { label: string; value: string; isDark: boolean; mono?: boolean }) {
  ...
  return (
    <div className="flex justify-between items-center text-sm py-2 border-b border-slate-100 dark:border-slate-700/50 last:border-0 last:pb-0">
      ...
      <span className={`font-semibold capitalize text-right max-w-[60%] break-words ${isDark ? "text-white" : "text-slate-900"} ${mono ? "font-mono uppercase tracking-wider text-xs" : ""}`}>
        {value}
      </span>
    </div>
  );
