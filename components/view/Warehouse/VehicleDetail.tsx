...
import {
  fetchVehicleDetail, updateVehicleStatus, publishVehicle, createAdminPayment,
  fetchInspectionsByVehicle, fetchRepairsByVehicle,
  clearSuccess, clearError, VehicleStatus,
} from "@/lib/state/slice/warehouse/warehouseSlice";
import Link from "next/link";
import toast from "react-hot-toast";
import { FiArrowLeft, FiTruck, FiClipboard, FiTool, FiDollarSign, FiExternalLink, FiCheck } from "react-icons/fi";
...
const VehicleDetail = ({ paramsPromise }: { paramsPromise: Promise<{ id: string }> }) => {
  const params = use(paramsPromise);
  const dispatch = useDispatch<AppDispatch>();
  const { selectedVehicle: vehicle, inspections, repairs, loading, actionLoading, error, successMessage } = useSelector(
  ...
  useEffect(() => {
    if (successMessage) { toast.success(successMessage); dispatch(clearSuccess()); dispatch(fetchVehicleDetail(params.id)); }
  ...
  const sc = statusConfig[vehicle.status] || { label: vehicle.status, color: "bg-slate-500/20 text-slate-400" };

  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      {/* Header */}
      <div className="flex items-start gap-3">
        <Link href="/warehouse/vehicles" className="p-2 rounded-xl bg-slate-800/50 hover:bg-slate-800 text-slate-400 transition-colors mt-1">
          <FiArrowLeft className="text-xl" />
        </Link>
        <div className="flex-1">
          <h1 className="text-2xl font-bold text-white">{vehicle.brandName} {vehicle.modelName}</h1>
          <div className="flex items-center gap-2 mt-1">
            <span className="font-mono text-emerald-400 text-sm bg-emerald-500/10 px-2 py-0.5 rounded">{vehicle.barcode}</span>
      ...
      <div className="flex flex-wrap gap-2">
        {vehicle.status === "inspecting" && (
          <Link href={`/warehouse/inspections/create?vehicleId=${vehicle.id}`}
        ...
        {vehicle.status === "registered" && (
          <button onClick={() => dispatch(createAdminPayment(vehicle.id))} disabled={actionLoading}
        ...
        {(vehicle.status === "in_warehouse" || vehicle.status === "in_repair") && (
          <>
            <Link href={`/warehouse/repairs/create?vehicleId=${vehicle.id}`}
            ...
            <button onClick={() => dispatch(updateVehicleStatus({ id: vehicle.id, status: "ready" }))} disabled={actionLoading}
        ...
        {vehicle.status === "ready" && (
          <button onClick={() => dispatch(publishVehicle(vehicle.id))} disabled={actionLoading}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <InfoCard title="Data Kendaraan" items={[
        ...
        <InfoCard title="Data Penjual & Harga" items={[
          ...
          ...(vehicle.notes ? [["Catatan", vehicle.notes] as [string, string]] : []),
        ]} />
      </div>
      ...
      <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6">
        <h3 className="text-lg font-semibold text-white mb-4 flex items-center gap-2">
          <FiClipboard className="text-yellow-400" /> Riwayat Inspeksi ({inspections.length})
        </h3>
        {inspections.length === 0 ? (
          <p className="text-slate-500 text-sm">Belum ada inspeksi</p>
        ) : (
              ...
              <div key={insp.id} className="bg-slate-700/30 rounded-xl p-4 border border-slate-700/50">
                ...
                <div className="grid grid-cols-5 gap-2 text-center text-xs">
                  {[
                    ...
                    <div key={label as string} className="bg-slate-700/50 rounded-lg p-2">
                ...
                <div className="mt-2 flex flex-wrap gap-2 text-xs">
                  {insp.hasBpkb && <span className="px-2 py-0.5 rounded bg-green-500/20 text-green-400">BPKB</span>}
                  {insp.hasStnk && <span className="px-2 py-0.5 rounded bg-green-500/20 text-green-400">STNK</span>}
                  {insp.hasFaktur && <span className="px-2 py-0.5 rounded bg-green-500/20 text-green-400">Faktur</span>}
                  {insp.hasKtp && <span className="px-2 py-0.5 rounded bg-green-500/20 text-green-400">KTP</span>}
                  {insp.hasSpareKey && <span className="px-2 py-0.5 rounded bg-green-500/20 text-green-400">Kunci Cadangan</span>}
                </div>
                {insp.repairNotes && <p className="text-sm text-orange-300 mt-2">Perbaikan: {insp.repairNotes}</p>}
                {insp.rejectionReason && <p className="text-sm text-red-300 mt-2">Alasan Tolak: {insp.rejectionReason}</p>}
              </div>
            ))}
          </div>
        )}
      ...
      <div className="bg-slate-800/50 border border-slate-700/50 rounded-2xl p-6">
        ...
        {repairs.length === 0 ? (
          <p className="text-slate-500 text-sm">Belum ada repair order</p>
        ) : (
          <div className="space-y-3">
            {repairs.map((r) => (
              <div key={r.id} className="bg-slate-700/30 rounded-xl p-4 border border-slate-700/50">
                <div className="flex justify-between items-start">
                  <div>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-medium ${
                      r.repairType === "light" ? "bg-blue-500/20 text-blue-400" : "bg-red-500/20 text-red-400"
                    }`}>{r.repairType === "light" ? "Ringan" : "Berat"}</span>
                    <span className={`ml-2 px-2 py-0.5 rounded-full text-xs font-medium ${
                      r.status === "completed" ? "bg-green-500/20 text-green-400" :
                      r.status === "in_progress" ? "bg-yellow-500/20 text-yellow-400" :
                      r.status === "cancelled" ? "bg-red-500/20 text-red-400" : "bg-slate-500/20 text-slate-400"
                    }`}>{r.status}</span>
                  </div>
                  <span className="text-xs text-slate-400">{formatDate(r.createdAt)}</span>
                </div>
                <p className="text-sm text-slate-300 mt-2">{r.description}</p>
                {r.estimatedCost && <p className="text-xs text-slate-400 mt-1">Estimasi: {formatPrice(r.estimatedCost)}</p>}
                {r.actualCost && <p className="text-xs text-emerald-400 mt-1">Aktual: {formatPrice(r.actualCost)}</p>}
              </div>
            ))}
          </div>
        )}
