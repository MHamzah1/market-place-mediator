  ...
  return (
    <div className="space-y-6 max-w-[1600px] mx-auto">
      ...
      >
        ...
        {/* Status Counts Bar */}
        <div
          className={`px-6 py-4 flex flex-wrap gap-3 ${isDark ? "bg-slate-800/30" : "bg-slate-50"}`}
        >
          <button
            onClick={() => {
              setStatusFilter("");
...
const VehicleListItem = ({
  ...
  return (
    <div
      onClick={onClick}
      className={`group flex gap-4 rounded-xl border p-3 cursor-pointer transition-all hover:shadow-md ${
        isDark
          ? "bg-slate-800/60 border-slate-700/50 hover:border-emerald-500/40"
          : "bg-white border-slate-200 hover:border-emerald-500/50"
      }`}
    >
      {/* Thumbnail */}
      <div className="w-28 h-20 rounded-lg overflow-hidden bg-slate-100 dark:bg-slate-900 flex-shrink-0">
        {imgUrl ? (
          <img
            src={imgUrl}
            alt={v.brandName}
  ...
  return (
    <div className="space-y-6">
      ...
      {detail.adminPayment && (
        <div>
          <h4
            className={`flex items-center gap-2 text-sm font-bold mb-3 ${isDark ? "text-blue-400" : "text-blue-600"}`}
          >
            <FiDollarSign /> Pembayaran Admin
          </h4>
          <div
          ...
          >
            <div className="flex justify-between items-center">
              <div>
                ...
                >
                  {detail.adminPayment.invoiceNumber}
                </p>
                <p
                  className={`text-xs ${isDark ? "text-slate-500" : "text-slate-400"}`}
                >
                  {formatPrice(detail.adminPayment.amount)} •{" "}
                  {detail.adminPayment.paymentStatus}
                </p>
              </div>
              <span
                className={`px-2.5 py-1 rounded-lg text-xs font-bold ${
                  detail.adminPayment.paymentStatus === "paid"
